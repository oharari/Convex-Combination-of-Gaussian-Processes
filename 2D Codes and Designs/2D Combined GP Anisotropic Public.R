...
#******* of Anisotropic Gaussian Processes to the output of computer experiments ******
#**************************************************************************************
...
#********************** and scale parameters theta1 and theta 2 ***********************
#**************************************************************************************
corr.matrix <- function(X, theta1, theta2)
{
	n <- dim(X)[1]
	d <- dim(X)[2]
	Theta <- diag(c(theta1, theta2))	
...
#**************************************************************************************
corr.vec <- function(x, X, theta1, theta2)
{
	n <- dim(X)[1]
	d <- dim(X)[2]
...
#**************************************************************************************
Mixed.corr.matrix <- function(D.train, p, theta1, theta2, lambda)
{
	R1 <- corr.matrix(D.train, theta1, theta2)
	R2 <- corr.matrix(D.train, (1+lambda)*theta1, (1+lambda)*theta2)
...
#********************************** taining set D.train *******************************
#**************************************************************************************
Mixed.corr.vec <- function(x.new, D.train, p, theta1, theta2, lambda)
{
	corr1 <- corr.vec(x.new, D.train, theta1, theta2)
	corr2 <- corr.vec(x.new, D.train, (1+lambda)*theta1, (1+lambda)*theta2)
...
logpost <- function(D.train, theta, y, sigma2)
{
	...
	R <- Mixed.corr.matrix(D.train, p, theta1, theta2, lambda)
	...
	log.jacob <- -phi -2*log(1 + exp(-phi)) + psi1 + psi2 + zeta

      #******* the log-prior density (go ahead and change it!) ********
	log.prior <- - psi1 - psi1^2/2 - psi2 - psi2^2/2 - 4*zeta - 4/lambda  # - 0.5*log(p*(1-p))
                          ...
                          y.train, net.samp.size)
{
	...
	theta2 <- exp(temp[,2])
	lambda <- exp(temp[,4])
	samp <- data.frame(cbind(p, theta1, theta2, lambda))
	names(samp) <- c("p", "theta1", "theta2", "lambda")
	...
	prediction.factors <- data.frame(t(apply(data.frame(cbind(R.Inv, beta)), 1, factors, 
                                               n.train = n.train, y.train = y.train)))
...
predict.post <- function(x.new, D.train, pars, sigma2)
{
	...
	lambda <- as.numeric(pars[4])
	beta <- as.numeric(pars[5])
	mean.factor <- as.numeric(pars[6:(5+n)])
	var.factor1 <- as.numeric(pars[(6+n):(5+2*n)])
	var.factor2 <- as.numeric(pars[(6+2*n)])
	R.Inv <- matrix(as.numeric(pars[(7+2*n):(6+2*n+n^2)]), nrow=n)	

	r <- matrix(Mixed.corr.vec(x.new, D.train, p, theta1, theta2, lambda), ncol = 1)
                            ...
                            alpha.geweke, batch.size, code)
{
	...
	plot(theta2, lwd = 2, main = '', ylim = ylims2, xlim = c(0,6))
	title(main = expression(paste("Distribution of ", theta[2])), cex.main = 1.5)
	lines(x, theta2.prior, lwd = 2, col = 2, lty = 2)
	legend('topright', col=c(2,1), lty = c(2,1), c("Prior", "Posterior"), lwd = 2, 
             cex = 1.2)

	plot(lambda, lwd = 2, main = '', ylim = ylims3, xlim = c(0,15))
	title(main = expression(paste("Distribution of ", lambda)), cex.main = 1.5)
	lines(x, lambda.prior, lwd = 2, col = 2, lty = 2)
	legend('topright', col=c(2,1), lty = c(2,1), c("Prior", "Posterior"), lwd = 2, 
             cex = 1.2)
...
start <- c(1,1,0,1) #a starting point for the Markov chain for the MCMC sampling
