...
prediction <- function(x.new, alpha, code, pars.frame, D.train, sigma2)
{
	...
	mean.sd <- as.numeric(batchSE(mcmc(pred.samp), batchSize = 20)[1])
	sd <- sqrt(mean(pred.samp$var) + mean.sd^2)
...
compare.GP <- function(D.new, alpha, code, params, D.train, sigma2, y.train)
{
	...
	pred <- predict(cgp, D.new, PI = TRUE)
	y.hat.CGP <- pred$Yp
	LL.CGP <- pred$Y_low
	UL.CGP <- pred$Y_up
...
func <- 2 #function number (originally 1-5)
