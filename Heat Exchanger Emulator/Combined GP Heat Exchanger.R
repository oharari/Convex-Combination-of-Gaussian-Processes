#**************************************************************************************
#***************** This is the R code for the Heat Exchanger emulator *****************
...
#********** based on the simulation designed and run in Qian et al. (2006) ************
#**************************************************************************************
...
#**************************************************************************************
logpost <- function(D.train, theta, y, sigma2, theta1.pars, theta2.pars)
{
	psi1 <- theta[1]
	psi2 <- theta[2]
	phi <- theta[3]
	...
	log.prior <- -(theta1.pars[1]+1)*psi1 -theta1.pars[2]/theta1 - (theta2.pars[1]+1)*psi2 - theta2.pars[2]/theta2 
...
#**************************************************************************************
Metro <- function(start, N, samp.size, batch.size, alpha, D.train, sigma2, y, theta1.pars, theta2.pars)
{
	...
	{
		return(logpost(D.train, theta, y, sigma2, theta1.pars, theta2.pars)$val)
	}
	...
	k = 1
	l.old <- logpost(D.train, theta.old, y, sigma2, theta1.pars, theta2.pars)
	pv = 0
	...
	{		
		...
		l.cand <- logpost(D.train, theta.candidate, y, sigma2, theta1.pars, theta2.pars)
...
choose.hyperpars <- function(D.train, y.train, hyperpars.matrix, sigma2)
{
	...
	for(i in 1:dim(hyperpars.matrix)[1])
	{
		...
		log.likes[i] <- log(likeli.hyperpars(D.train, y.train, hyperpars.matrix[i,1:2], 
...
factors.frame <- function(start, N, samp.size, batch.size, alpha.geweke, D.train, 
                          sigma2, y.train, net.samp.size, theta1.pars, theta2.pars)
{
	n.train <- dim(D.train)[1]

	s <- Metro(start, N, samp.size, batch.size, alpha.geweke, D.train, sigma2, y.train, theta1.pars, theta2.pars)
...
prediction <- function(x.new, alpha, pars.frame, D.train, sigma2)
{
	...
	return(c(y.hat.Combined, LL.Combined, UL.Combined))
}
#**************************************************************************************
...
compare.GP <- function(D.test, alpha, y.test, params, D.train, sigma2, y.train)
{
	...
	names(predict) <- c("y.hat.Combined", "LL.Combined", "UL.Combined")
	y.true <- y.test
	
	return(data.frame(cbind(D.test, predict, y.true)))
}
#**************************************************************************************
...
start <- c(1,2.7,0) #a starting point for the Markov chain for the MCMC sampling
...
#insert here the directory in which the training and test sets are located
wd = dirname(rstudioapi::getActiveDocumentContext()$path)
setwd(wd)

train.data <- read.table("Qian Training Set.txt", header = T)
test.data <- read.table("Qian Test Set.txt", header = T)	
...
#theta1.pars <- hyperpars$pars[1:2]
#theta2.pars <- hyperpars$pars[3:4]
#cbind(hyperpars.matrix, hyperpars$likelihoods)
...
params <- factors.frame(start, N.max, samp.size, batch.size, alpha.geweke, 
				D.train, sigma2, y.train, net.samp.size, theta1.pars, theta2.pars)
...
p.prior <- dbeta(t, 1, 1) 
theta1.prior <- dinvgamma(x, theta1.pars[1], theta1.pars[2])
theta2.prior <- dinvgamma(x, theta2.pars[1], theta2.pars[2])
...
plot(p, lwd = 3, main = '', ylim = ylims0, xlim = c(0.6,.95), xlab = '', cex.axis = 1.8, ylab = '')
title(main = expression(paste("Distribution of ", p)), cex.main = 2.5)
lines(t, p.prior, col = 2, lty = 2, lwd = 3)
legend('topleft', col=c(2,1), lty = c(2,1), c("Prior", "Posterior"), lwd = 3, cex = 2)
...
legend('topright', col=c(2,1), lty = c(2,1), c("Prior", "Posterior"), lwd = 3, cex = 2)
...
e.Combined <- y.true - y.hat.Combined
RMSPE.Combined <- sqrt(mean(e.Combined^2))

y.hat.single <- predict.gp(ord, D.test, se.fit = FALSE)
e.Single <- y.true - y.hat.single
...
par(mar = c(5,5,1,1))
plot(y.true, y.hat.Combined, pch = 19, cex.lab = 2, xlab = 'True Values', ylab = 'Predicted Values', cex = 1.3)
abline(c(0,1), col = 2, lty = 2, lwd = 2)
